...
/*   Updated: 2022/05/24 19:51:40 by kkaczoro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_no_new_line(int fd, char *buffer)
{
	...
	temp = malloc(sizeof(char) * (ft_strlen(buffer)) + 1);//allocation likely unnecessary
	if (temp == NULL)
		return (NULL);
	(void)ft_strlcpy(temp, buffer, ft_strlen(buffer) + 1);
	len_read = read(fd, buffer, BUFFER_SIZE);//THIS SECTION (from len_nezt until end of following if statement) IS SUS AND PROLLY UNENCESSARY
	if (len_read <= 0)
	{
	...
	if (len_read <= BUFFER_SIZE)
		(void)ft_memset(buffer + len_read, '\0', BUFFER_SIZE - len_next);
...
static char	*ft_fuse(char *temp, char *new_line)
{
	...
	next_line = malloc(sizeof(char) * (ft_strlen(temp) + ft_strlen(new_line)) + 1);//if split into different functions: change new_line name to smth more easy to comprehend
	if (next_line == NULL)
	{
		free (temp);
		free (new_line);
	...
	(void)ft_strlcpy(next_line + ft_strlen(temp), new_line, ft_strlen(new_line) + 1);
	free (temp);
	free (new_line);
	return (next_line);
}
